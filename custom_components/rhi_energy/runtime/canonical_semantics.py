...
def prop(
    ...
    *,
    ...
    write = {
        ...
        "service": "rhi_energy.write_property" if editable else None,
...
def overview_snapshot(facts: dict[str, Any], demand_breakdown: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "ownership": {
            "status_and_primary_metric": "energy:contract:overview",
            "physical_facts": "energy:canonical:properties",
...
def deterministic_plan(facts: dict[str, Any], settings: dict[str, Any], flexible_assets: list[dict[str, Any]], now: datetime | None = None) -> dict[str, Any]:
    ...
    def build_horizon(
        ...
        return ({
            ...
            "baseline_demand": {"home_kwh": demand},
            "flexible_demand": {"known_need_kwh": requested},
            "scheduled": {"flexible_kwh": scheduled},
            "remaining": {"flexible_kwh": deferred},
            "price_value": {
                "import_price_eur_kwh": import_price,
                "planned_import_cost_eur": round(grid_import * import_price, 4) if grid_import is not None and import_price is not None else None,
                "quality": "estimated" if grid_import is not None and import_price is not None else "incomplete",
            },
            "balance": {
            ...
            "source_refs": [
                "energy:property:forecast.solar_remaining_today_kwh" if horizon_id == "D0" else "energy:property:forecast.solar_tomorrow_kwh",
                "energy:object:home_consumption",
                "energy:contract:flexible_plan",
                "energy:configuration:strategy",
                "energy:configuration:pricing",
            ],
