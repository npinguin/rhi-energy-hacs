...
def _metering_projection(store_data: dict[str, Any]) -> tuple[str, dict[str, Any], list[dict[str, Any]]]:
    ...
    field_keys = (
        "solar_kwh", "grid_import_kwh", "grid_export_kwh", "site_consumption_kwh",
        "home_consumption_kwh", "battery_charge_kwh", "battery_discharge_kwh",
        "flexible_loads_energy_in_kwh",
    )
    properties_by_period: dict[str, list[dict[str, Any]]] = {}
    for period_id, period in by_id.items():
        period_status = str(period.get("availability") or UNAVAILABLE)
        field_quality = period.get("field_quality") or {}
        period_props = []
        for key in field_keys:
            quality = str(field_quality.get(key) or period_status)
            field_availability = AVAILABLE if quality == "OK" else quality
            period_props.append(
                prop(
                    "metering",
                    f"metering.{period_id}.{key}",
                    period.get(key),
                    "kWh",
                    availability=field_availability,
                    reason_code=f"METERING_{quality}",
                )
            )
        properties_by_period[period_id] = period_props
    props = selector_props + deepcopy(properties_by_period.get(selected) or [])
    return status, {
        "selected_period_id": selected,
        "periods": rows,
        "periods_by_id": by_id,
        "selected": current,
        "properties_by_period_id": properties_by_period,
    }, props
...
def project_all(snapshot: dict[str, Any], store_data: dict[str, Any], command_rows: list[dict[str, Any]], manager: Any) -> dict[str, dict[str, Any]]:
    ...
    value_contexts_by_period: dict[str, dict[str, Any]] = {}
    for period_id, period in (meter_ctx.get("periods_by_id") or {}).items():
        period_status, period_rows, period_summary = _value_projection(period, pricing_rows)
        value_contexts_by_period[str(period_id)] = {
    ...
    records_by_period = {
        period_id: [row for row in asset_meter_records if row["period"] == period_id]
        for period_id in ("hour", "today", "week", "month", "year")
    }
    selected_records=records_by_period.get(selected_period) or []
    projections["energy_asset_metering_index"]={"state":AVAILABLE if any(row.get("energy_kwh") is not None for row in selected_records) else UNAVAILABLE,"attributes":{**_base("asset_metering_index_v3_explicit_render_contract"),"index_type":"asset_metering_index","schema_version":6,"selected_period":selected_period,"source_owner":"sensor.energy_metering_property_index","recalculation_forbidden":True,"records_json":jdump(asset_meter_records),"selected_records_json":jdump(selected_records),"records_by_period_json":jdump(records_by_period),"records_by_period_id":jdump(records_by_period),"period_summary_by_id":jdump(meter_ctx["periods_by_id"]),"selected_period_summary_json":jdump(meter_ctx["selected"]),"applicable_remediations_json":jdump([]),"render_contract_json":jdump({"source":"records_json filtered by period_id","selected_source":"selected_records_json","indexed_source":"records_by_period_json[period_id]","value":"energy_kwh","state":"measurement_state","status":"status_label","zero_is_measured":True,"ux_recalculation_allowed":False}),"status_semantics":"Every supported period is published concurrently; selected_period controls only the compatibility default view."}}
